...
function SongList({ defaultLevel = 'ez' }) {
	...
	function switchSong(id) {
		...
		fetch(
			`https://charts-phi.pages.dev/${codename}/${songMeta['illustration']}`
		)
			...
			.catch((err) => {
				console.err(
					'获取曲绘失败!',
					`url: https://charts-phi.pages.dev/${codename}/${songMeta['illustration']}`,
					err
				);
			});
		fetch(
			`https://charts-phi.pages.dev/${codename}/${songMeta['musicFile']}`
		)
			.then((response) => response.blob())
			.then((blob) => {
			...
			.catch((err) => {
				console.err(
					'获取歌曲失败!',
					`url: https://charts-phi.pages.dev/${codename}/${songMeta['musicFile']}`,
					err
				);
			});
